...
class TableStr(IMDB):
    def __init__(self, image_set, root_path, devkit_path, result_path=None, mask_size=-1, binary_thresh=None):
        ...
        print (image_set)
        super(TableStr, self).__init__(self.__class__.__name__, image_set, root_path, devkit_path, result_path)  # set self.name
    ...
    def load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            gt_classes[ix] = cls
            overlaps[ix, cls] = 1.0
            print ("Current bbox: %s" %str(boxes[ix, :]))
            print ("Number of objects found: %d" %(ix))
            print ("Files loaded %s :"  % (roi_rec))
        ...
        #print 'load roi_rec: {}'.format(filename)
        #print ("Files loaded %s :"  % len(roi_rec))
        #print ("Files loaded : %s" %(filename))
