...
class resnet_v1_101_rcnn_dcn_dense(Symbol):
    def __init__(self):
        """
    ...
    def get_resnet_v1_conv4(self, data):
        ...
        return conv1_relu, res2c_relu, res3b3_relu, res4b10_relu, res4b22_relu
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        conv1_relu, res2c_relu, res3b3_relu, res4b10_relu, conv_feat = self.get_resnet_v1_conv4(data)
        # res5
        relu1 = self.get_resnet_v1_conv5(conv_feat)

        # TODO: Possible position for combination of all local features with the relu_1
        # mx.sym.ROIPooling(data=conv1_relu, rois=[[]], pooled_size=desiredShape, spatial_scale=1.0)
        ...
        # TODO: Possible position for combination of all local features with the conv_new_1_relu
        ...
        deformable_roi_pool = mx.contrib.sym.DeformablePSROIPooling(name='deformable_roi_pool', data=conv_new_1_relu, rois=rois,
                                                                    ...
                                                                    no_trans=False, part_size=7, output_dim=256, spatial_scale=1.0/16.0, trans_std=0.1)
                                                                    # no_trans=False, part_size=7, output_dim=256, spatial_scale=0.0625, trans_std=0.1)


        # TODO: Possible position for combination of all local features with the conv_new_1_relu
        filterSizeForConvs = (3, 3)
        filterPadForConvs = (1, 1)
        conv1_relu = mx.symbol.Convolution(data=conv1_relu, kernel=filterSizeForConvs, pad=filterPadForConvs,
                                        stride=(1, 1), num_filter=256, name="conv1_relu_256conv", no_bias=True)
        conv1_relu = mx.symbol.Activation(data=conv1_relu, act_type='relu', name='conv1_relu_256conv_relu')
        conv1_relu_pooled = mx.symbol.ROIPooling(name='roi_pool_conv1', data=conv1_relu, rois=rois, pooled_size=(7,7), spatial_scale=1.0/2.0)

        res2c_relu = mx.symbol.Convolution(data=res2c_relu, kernel=filterSizeForConvs, pad=filterPadForConvs,
                                        stride=(1, 1), num_filter=256, name="res2c_relu_256conv", no_bias=True)
        res2c_relu = mx.symbol.Activation(data=res2c_relu, act_type='relu', name='res2c_relu_256conv_relu')
        res2c_relu_pooled = mx.symbol.ROIPooling(name='roi_pool_res2c', data=res2c_relu, rois=rois, pooled_size=(7,7), spatial_scale=1.0/4.0)

        res3b3_relu = mx.symbol.Convolution(data=res3b3_relu, kernel=filterSizeForConvs, pad=filterPadForConvs,
                                        stride=(1, 1), num_filter=256, name="res3b3_relu_256conv", no_bias=True)
        res3b3_relu = mx.symbol.Activation(data=res3b3_relu, act_type='relu', name='res3b3_relu_256conv_relu')
        res3b3_relu_pooled = mx.symbol.ROIPooling(name='roi_pool_res3b3', data=res3b3_relu, rois=rois, pooled_size=(7,7), spatial_scale=1.0/8.0)

        res4b10_relu = mx.symbol.Convolution(data=res4b10_relu, kernel=filterSizeForConvs, pad=filterPadForConvs,
                                        stride=(1, 1), num_filter=256, name="res4b10_relu_256conv", no_bias=True)
        res4b10_relu = mx.symbol.Activation(data=res4b10_relu, act_type='relu', name='res4b10_relu_256conv_relu')
        res4b10_relu_pooled = mx.symbol.ROIPooling(name='roi_pool_res', data=res4b10_relu, rois=rois, pooled_size=(7,7), spatial_scale=1.0/16.0)

        conv_feat = mx.symbol.Convolution(data=conv_feat, kernel=filterSizeForConvs, pad=filterPadForConvs,
                                        stride=(1, 1), num_filter=256, name="conv_feat_256conv", no_bias=True)
        conv_feat = mx.symbol.Activation(data=conv_feat, act_type='relu', name='conv_feat_256conv_relu')
        conv_feat_pooled = mx.symbol.ROIPooling(name='roi_pool_res2c', data=conv_feat, rois=rois, pooled_size=(7,7), spatial_scale=1.0/16.0)

        # Concatenate the different features
        # deformable_roi_pool_new = mx.sym.concat(conv1_relu_pooled, res2c_relu_pooled, res3b3_relu_pooled, res4b10_relu_pooled, conv_feat_pooled, deformable_roi_pool, dim=1)
        # res4b22 = mx.symbol.broadcast_add(name='res4b22', *[res4b21_relu, scale4b22_branch2c])
        deformable_roi_pool_new = mx.symbol.add_n(conv1_relu_pooled, res2c_relu_pooled, res3b3_relu_pooled, res4b10_relu_pooled, conv_feat_pooled, deformable_roi_pool, name='features_add')

        # 2 fc
        # fc_new_1 = mx.sym.FullyConnected(name='fc_new_1', data=deformable_roi_pool, num_hidden=1024)
        fc_new_1 = mx.sym.FullyConnected(name='fc_new_1', data=deformable_roi_pool_new, num_hidden=1024)
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        ...
        # New params
        arg_params['conv1_relu_256conv_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['conv1_relu_256conv_weight'])
        # arg_params['conv1_relu_256conv_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['conv1_relu_256conv_bias'])

        arg_params['res2c_relu_256conv_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['res2c_relu_256conv_weight'])
        # arg_params['res2c_relu_256conv_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['res2c_relu_256conv_bias'])

        arg_params['res3b3_relu_256conv_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['res3b3_relu_256conv_weight'])
        # arg_params['res3b3_relu_256conv_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['res3b3_relu_256conv_bias'])

        arg_params['res4b10_relu_256conv_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['res4b10_relu_256conv_weight'])
        # arg_params['res4b10_relu_256conv_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['res4b10_relu_256conv_bias'])

        arg_params['conv_feat_256conv_weight'] = mx.random.normal(0, 0.001, shape=self.arg_shape_dict['conv_feat_256conv_weight'])
        # arg_params['conv_feat_256conv_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['conv_feat_256conv_bias'])
